        ...
        <form action="{{ route('pupuk.update', $pupuk->pupuk_id) }}" method="POST">
            ...
            <div class="mb-3">
                ...
                <div class="input-group">
                    ...
                    <input type="date" name="pupuk_tanggal_pakai"
                        value="{{ old('pupuk_tanggal_pakai', $pupuk->pupuk_tanggal_pakai) }}"
                        class="form-control @error('pupuk_tanggal_pakai') is-invalid @enderror" required>
            ...
            <div class="mb-3">
                ...
                <div class="input-group">
                    ...
                    <input type="text" name="pupuk_jenis" value="{{ old('pupuk_jenis', $pupuk->pupuk_jenis) }}"
                        class="form-control @error('pupuk_jenis') is-invalid @enderror" required>
                    @error('pupuk_jenis')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
            ...
            <div class="mb-3">
                ...
                <div class="input-group">
                    ...
                    <input type="number" step="0.01" name="pupuk_jumlah_pakai"
                        value="{{ old('pupuk_jumlah_pakai', $pupuk->pupuk_jumlah_pakai) }}"
                        class="form-control @error('pupuk_jumlah_pakai') is-invalid @enderror" required>
                            ...
                            <select name="mt_id" class="form-select @error('mt_id') is-invalid @enderror" required>
                                @foreach ($mtPetani as $mt)
                                    <option value="{{ $mt->mt_id }}"
                                        {{ old('mt_id', $pupuk->mt_id) == $mt->mt_id ? 'selected' : '' }}>
                                        {{ $mt->mt_komoditas }} - {{ $mt->kebun->kebun_nama }}
                                    </option>
                                @endforeach
                            </select>
                        @else
                            <input type="text" class="form-control" value="Tidak ada musim tanam milik Anda" readonly>
                        @endif
                    @else
                        <select name="mt_id" class="form-select @error('mt_id') is-invalid @enderror" required>
